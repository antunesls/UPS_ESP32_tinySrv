...
static float linear(uint8_t val, float a, float b)
{
...
static void processPower(float wOut, float wIn, float *power_current_kwh_out, float *power_current_kwh_in, int64_t *power_last_update)
{
...
static void build_payload(const uint8_t *buffer, char *output_json, size_t output_size)
{
...
static void handle_newdev(usb_device_handle_t usb_dev)
{

    // Libere o dev_desc anterior, se houver
    if (dev_desc != NULL)
    {
        free(dev_desc);
        dev_desc = NULL;
